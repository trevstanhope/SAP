...
class StateMachine(object):
    ...
    def __init__(self, object):
        ...
        try:
            ...
            self.ALIGN_RIGHT_COMMAND = object.ALIGN_RIGHT_COMMAND
            self.ALIGN_CENTER_COMMAND = object.ALIGN_CENTER_COMMAND
            self.ALIGN_LEFT_COMMAND = object.ALIGN_LEFT_COMMAND
            ### Alignment
            self.right_aligned = False
            self.left_aligned = False
            print('\tOKAY')
        except Exception as error:
            print('\tERROR: %s' % str(error))
    ...
    def command_scout_left(self, request):
        ...
        try:
            ...
            ### 1. If not aligned ...
            if not self.left_aligned:
				command = self.ALIGN_CENTER_COMMAND
				self.left_aligned = True
            ### 2. If not enough trees detected ...
            elif len(self.true_path) < self.NUM_TREES:
                self.draw_left_tree_map(trees_snapshot)
                command = self.WAIT_COMMAND
            ### 3. If enough trees detected ...
            else:
    ...
    def update_tree_map(self):
        ...
        try:
            tree_map = 255 * (self.left_tree_map + self.right_tree_map) # (self.left_tree_map + self.center_tree_map + self.right_tree_map)
            return tree_map
        except Exception as error:
            print('\tERROR: %s' % str(error))
    ...
    def draw_left_tree_map(self, snapshot):
        print('\t\t[Drawing Left Tree Map] %s' % datetime.strftime(datetime.now(), "%Y-%m-%d %H:%M:%S"))
        self.left_tree_map = numpy.zeros([self.X_SIZE, self.Y_SIZE])
        (x, y, t) = self.scout_left_position
        x_all = []
        y_all = []
        for offset in snapshot:
            for error in numpy.linspace(-0.03, 0.03, 10):
    ...
    def draw_center_tree_map(self, snapshot):
        ...
        y_all = []
        for offset in snapshot:
            for error in numpy.linspace(-0.02, 0.02, 4):
                x1 = x + numpy.cos(t + offset + error) * self.DIST_MIN
                y1 = y + numpy.sin(t + offset + error) * self.DIST_MIN
                if (x1 >= self.X_SIZE - 2) or (y1 >= self.Y_SIZE - 2):
                    pass
    ...
    def predict_trees(self, tree_map):
        ...
        self.detected_trees = detected_trees
        print('\tTrees: %s' % str(detected_trees))
    ...
    def find_path(self, trees):
        ...
        try:
            ...
            third_tree_paths = []
            for i in range(len(trees)):
                d = self.dist(second_tree, trees[i])
                third_tree_paths.append((d,i))
            (min_third_tree_dist, min_third_tree_index) = min(third_tree_paths)
            third_tree = trees[min_third_tree_index]
            trees.pop(min_third_tree_index)
            ## Find Fourth Tree
            fourth_tree_paths = []
            for i in range(len(trees)):
                d = self.dist(third_tree, trees[i])
                fourth_tree_paths.append((d,i))
            (min_fourth_tree_dist, min_fourth_tree_index) = min(fourth_tree_paths)
            fourth_tree = trees[min_fourth_tree_index]
            trees.pop(min_fourth_tree_index)
            ## Find Fifth Tree
            fifth_tree_paths = []
            for i in range(len(trees)):
                d = self.dist(fourth_tree, trees[i])
                fifth_tree_paths.append((d,i))
            (min_fifth_tree_dist, min_fifth_tree_index) = min(fifth_tree_paths)
            fifth_tree = trees[min_fifth_tree_index]
            trees.pop(min_fifth_tree_index)
            ...
            try:
                ...
                for p in it.permutations(trees):
                    p = p + (fifth_tree, fourth_tree, third_tree, second_tree, first_tree, (48,1))
                    paths.append(p)
            except Exception as error:
